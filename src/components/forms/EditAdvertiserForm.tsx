...
interface EditAdvertiserFormProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: AdvertiserFormData) => void
  advertiser?: AdvertiserFormData & { id: string }
}
...
const EditAdvertiserForm: React.FC<EditAdvertiserFormProps> = ({
  isOpen,
  onClose,
  onSubmit,
  advertiser
  ...
  // Заполняем форму данными рекламодателя при открытии
  useEffect(() => {
    if (advertiser && isOpen) {
      setFormData({
        ...
        legalAddress: advertiser.legalAddress || '',
        bik: advertiser.bik || '',
        account: advertiser.account || '',
        contractNumber: advertiser.contractNumber || '',
        contractDate: advertiser.contractDate || '',
        useAutoMarking: advertiser.useAutoMarking || false
      })
    }
  }, [advertiser, isOpen])
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Редактировать рекламодателя</h2>
          <button
            onClick={onClose}
